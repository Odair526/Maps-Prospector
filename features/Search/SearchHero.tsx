...
import { MapPin, Navigation, Briefcase, Target, MessageCircle, Globe, Instagram, Facebook, Linkedin, Check, Zap, Search, Clock, Loader2, Info } from 'lucide-react';
      ...
      <div className="flex flex-col items-center mb-10">
        ...
        {(state === AppState.SEARCHING || (elapsedTime > 0 && state !== AppState.IDLE)) && (
          <div className={`mb-4 backdrop-blur border rounded-full px-4 py-1 text-sm font-mono flex items-center gap-2 shadow-sm animate-in fade-in ${isDarkMode ? 'bg-slate-800/50 border-slate-700 text-gray-300' : 'bg-white/80 border-gray-200 text-gray-600'}`}>
             ...
             <span>{state === AppState.SEARCHING && !params.fastMode ? 'Varredura Exaustiva: ' : 'Tempo: '}{formatTime(elapsedTime)}</span>
          </div>
        )}
        ...
        <form id="search-form" onSubmit={(e) => onSearch(e, false)} className="space-y-6 relative z-10">
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1 flex flex-col gap-2">
              <button type="button" onClick={(e) => onSearch(e, true)} disabled={state === AppState.SEARCHING} className={`w-full flex items-center justify-center gap-3 py-4 px-6 rounded-2xl font-bold text-lg transition-all ${state === AppState.SEARCHING ? 'opacity-50 cursor-not-allowed' : (isDarkMode ? 'bg-amber-900/40 text-amber-400 border border-amber-900/50' : 'text-amber-700 bg-amber-50 border border-amber-200')}`}>
                {state === AppState.SEARCHING && params.fastMode ? <Loader2 className="w-5 h-5 animate-spin" /> : <Zap className="w-5 h-5" />}
                <span>Rápida</span>
              </button>
              <p className="text-[10px] text-center text-gray-400 uppercase font-bold tracking-tighter">~15 seg | 5-8 leads</p>
            </div>
            
            <div className="flex-[2] flex flex-col gap-2">
              <button type="submit" disabled={state === AppState.SEARCHING} className={`w-full flex items-center justify-center gap-3 py-4 px-6 rounded-2xl text-white font-bold text-lg transition-all relative overflow-hidden group ${state === AppState.SEARCHING ? 'bg-slate-300' : 'bg-gradient-to-r from-blue-600 to-cyan-600 shadow-xl shadow-blue-500/20'}`}>
                ...
                <span>{state === AppState.SEARCHING ? 'Mapeando Área...' : 'Iniciar Prospecção'}</span>
              </button>
              <div className="flex items-center justify-center gap-1 text-[10px] text-blue-500 font-bold uppercase tracking-tighter">
                <Info className="w-3 h-3" />
                <span>Modo Completo (Gemini 3 Pro) | 20-30 leads | 1-3 min</span>
              </div>
            </div>
          </div>
        </form>
