...
import { Search, Download, MapPin, Briefcase, Target, Navigation, MessageCircle, XCircle, Sparkles, History, Trash2, RotateCcw, X, Clock, LogOut, User as UserIcon, PlusCircle, Loader2, Zap, StopCircle, Globe, Instagram, Facebook, Linkedin, Check, Moon, Sun, Filter, Hash, Star, ThumbsUp, ThumbsDown, PhoneOff } from 'lucide-react';
...
export default function App() {
  ...
  // UPDATED: High Contrast for Light Mode. Clean whites and distinct borders.
  ...
  return (
    <div className={`min-h-screen relative overflow-hidden selection:bg-blue-500 selection:text-white font-sans transition-colors duration-300 ${isDarkMode ? 'bg-slate-950' : 'bg-gray-50'}`}>
      ...
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden z-0 pointer-events-none">
        <div className={`absolute top-[-10%] right-[-5%] w-[40rem] h-[40rem] rounded-full blur-[100px] opacity-40 animate-pulse ${isDarkMode ? 'bg-blue-900/30' : 'bg-blue-300/20'}`} />
        <div className={`absolute bottom-[-10%] left-[-10%] w-[35rem] h-[35rem] rounded-full blur-[100px] opacity-40 animate-pulse delay-1000 ${isDarkMode ? 'bg-purple-900/30' : 'bg-purple-300/20'}`} />
        <div className={`absolute top-[40%] left-[20%] w-[20rem] h-[20rem] rounded-full blur-[80px] opacity-30 ${isDarkMode ? 'bg-cyan-900/20' : 'bg-cyan-200/20'}`} />
               ...
               <div className="relative">
                 ...
                 {isProfileMenuOpen && (
                   <>
                     ...
                     <div className={`absolute right-0 top-full mt-2 w-48 rounded-xl shadow-xl border p-1 animate-in fade-in slide-in-from-top-1 z-50 ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
                        <button 
                          onClick={() => {
                            setShowProfileModal(true);
                            setIsProfileMenuOpen(false);
                          }}
                          className={`w-full flex items-center gap-2 px-3 py-2 text-sm rounded-lg transition-colors mb-1 ${isDarkMode ? 'text-gray-200 hover:bg-slate-700' : 'text-gray-700 hover:bg-gray-100'}`}
                        >
      ...
      <div className={`fixed inset-0 z-50 transform transition-all duration-500 ease-in-out ${showHistory ? 'translate-x-0' : 'translate-x-full pointer-events-none'}`}>
        ...
        <div className={`absolute top-0 right-0 h-full w-full max-w-sm backdrop-blur-2xl shadow-2xl border-l flex flex-col transform transition-transform duration-500 ${isDarkMode ? 'bg-slate-900/95 border-slate-700' : 'bg-white/95 border-gray-200'}`}>
                ...
                searchHistory.map((item) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-2">
                       ...
                       <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full whitespace-nowrap ${isDarkMode ? 'bg-slate-700 text-gray-400' : 'bg-gray-100 text-gray-600'}`}>
                    ...
                    <div className={`flex items-center justify-between mt-2 pt-2 border-t ${isDarkMode ? 'border-slate-700' : 'border-gray-100'}`}>
           ...
           {searchHistory.length > 0 && (
             <div className={`p-4 border-t ${isDarkMode ? 'border-slate-800 bg-slate-900' : 'border-gray-200 bg-gray-50'}`}>
                <button 
      ...
      <main className="relative z-10 flex-grow flex flex-col items-center w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                       ...
                       <button
                         ...
                         className={`flex items-center justify-between p-3 rounded-xl border transition-all active:scale-95 hover:scale-[1.02] select-none 
                            ${params.deepSearchWeb 
                                ...
                                : (isDarkMode ? 'bg-slate-800 border-slate-700 text-gray-400 hover:bg-slate-700' : 'bg-white border-gray-300 text-gray-600 hover:bg-gray-50')}`}
                       >
                ...
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-3 py-4 px-6 rounded-2xl font-bold text-lg transition-all shadow-sm active:translate-y-0.5 transform duration-200 group ${
                    (state === AppState.SEARCHING || isLoadingMore)
                      ...
                      : (isDarkMode ? 'bg-amber-900/40 text-amber-400 border border-amber-900/50 hover:bg-amber-900/60' : 'text-amber-700 bg-amber-50 hover:bg-amber-100 hover:shadow-md border border-amber-200')
                  }`}
                ...
                <button
                  ...
                  className={`flex-[2] flex items-center justify-center gap-3 py-4 px-6 rounded-2xl text-white font-bold text-lg transition-all shadow-lg hover:-translate-y-0.5 active:translate-y-0 transform duration-200 relative overflow-hidden group ${
                    (state === AppState.SEARCHING || isLoadingMore)
                      ...
                      : 'bg-gradient-to-r from-blue-600 to-cyan-600 hover:shadow-blue-500/30 border border-transparent'
                  }`}
                >
                  {/* Button Shine Effect */}
        ...
        {state === AppState.ERROR && (
          <div className={`w-full max-w-2xl backdrop-blur p-6 rounded-2xl border text-center mb-12 shadow-lg animate-in fade-in slide-in-from-bottom-2 ${isDarkMode ? 'bg-red-900/30 border-red-900/50 text-red-300' : 'bg-red-50/90 border-red-100 text-red-600'}`}>
          ...
          <div className="w-full animate-in fade-in slide-in-from-bottom-8 duration-700 pb-16 flex flex-col items-center">
            ...
            <div className="flex flex-col md:flex-row items-center justify-between w-full mb-6 gap-4">
              <div className="flex items-center gap-3">
                 ...
                 <span className={`text-sm font-bold px-3 py-1 rounded-full shadow-lg ${isDarkMode ? 'bg-slate-700 text-white shadow-black/20' : 'bg-gray-800 text-white shadow-gray-500/10'}`}>
              ...
              <div className="flex flex-col lg:flex-row gap-6">
                ...
                <div className="flex-1 space-y-4">
                   ...
                   <div className="flex flex-wrap gap-2 items-center">
                     <span className={`text-xs font-semibold uppercase mr-1 ${isDarkMode ? 'text-gray-500' : 'text-gray-500'}`}>WhatsApp:</span>
                     <div className="flex gap-2">
                        <button 
                          onClick={() => setResultFilters({...resultFilters, whatsappMode: 'all'})}
                          className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all active:scale-95 ${resultFilters.whatsappMode === 'all' ? (isDarkMode ? 'bg-blue-900/50 text-blue-300 border border-blue-800' : 'bg-blue-100 text-blue-700 border border-blue-200') : (isDarkMode ? 'bg-slate-700 text-gray-400' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50')}`}
                        >
                          Todos
                        </button>
                        <button 
                          onClick={() => setResultFilters({...resultFilters, whatsappMode: 'with_whatsapp'})}
                          className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all active:scale-95 flex items-center gap-1.5 ${resultFilters.whatsappMode === 'with_whatsapp' ? (isDarkMode ? 'bg-green-900/50 text-green-400 border border-green-800' : 'bg-green-100 text-green-700 border border-green-200') : (isDarkMode ? 'bg-slate-700 text-gray-400' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50')}`}
                        >
                          <MessageCircle className="w-3 h-3" /> Com Zap
                        </button>
                        <button 
                          onClick={() => setResultFilters({...resultFilters, whatsappMode: 'no_whatsapp'})}
                          className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all active:scale-95 flex items-center gap-1.5 ${resultFilters.whatsappMode === 'no_whatsapp' ? (isDarkMode ? 'bg-red-900/50 text-red-400 border border-red-800' : 'bg-red-100 text-red-700 border border-red-200') : (isDarkMode ? 'bg-slate-700 text-gray-400' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50')}`}
                        >
                   ...
                   <div className="flex flex-wrap gap-2 items-center">
                     <span className={`text-xs font-semibold uppercase mr-1 ${isDarkMode ? 'text-gray-500' : 'text-gray-500'}`}>Avaliação:</span>
                     <div className="flex gap-2">
                        <button 
                           onClick={() => setResultFilters({...resultFilters, ratingMode: 'all'})}
                           className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all active:scale-95 ${resultFilters.ratingMode === 'all' ? (isDarkMode ? 'bg-blue-900/50 text-blue-300 border border-blue-800' : 'bg-blue-100 text-blue-700 border border-blue-200') : (isDarkMode ? 'bg-slate-700 text-gray-400' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50')}`}
                         >
                           Todas
                         </button>
                         <button 
                           onClick={() => setResultFilters({...resultFilters, ratingMode: 'positive'})}
                           className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all active:scale-95 flex items-center gap-1.5 ${resultFilters.ratingMode === 'positive' ? (isDarkMode ? 'bg-yellow-900/50 text-yellow-400 border border-yellow-800' : 'bg-yellow-100 text-yellow-700 border border-yellow-200') : (isDarkMode ? 'bg-slate-700 text-gray-400' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50')}`}
                         >
                           <Star className="w-3 h-3 fill-current" /> +4.0
                         </button>
                         <button 
                           onClick={() => setResultFilters({...resultFilters, ratingMode: 'negative'})}
                           className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all active:scale-95 flex items-center gap-1.5 ${resultFilters.ratingMode === 'negative' ? (isDarkMode ? 'bg-orange-900/50 text-orange-400 border border-orange-800' : 'bg-orange-100 text-orange-700 border border-orange-200') : (isDarkMode ? 'bg-slate-700 text-gray-400' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50')}`}
                         >
                ...
                <div className="flex-1 space-y-3 min-w-[200px]">
                   ...
                   <div className="relative">
                      ...
                      <input 
                        ...
                        className={`w-full pl-9 pr-3 py-2 text-sm rounded-lg border outline-none transition-all ${isDarkMode ? 'bg-slate-900/50 border-slate-600 text-gray-200 placeholder-gray-600 focus:border-blue-500' : 'bg-white border-gray-300 text-gray-900 placeholder-gray-500 focus:border-blue-500'}`}
                      />
                   </div>
